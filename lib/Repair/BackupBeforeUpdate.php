...
/**
 * Pre-migration repair step: snapshot BudgetCheck data before schema migrations run.
 *
...
final class BackupBeforeUpdate implements IRepairStep
{
	...
	{
		return 'Back up BudgetCheck data before update migrations';
	}

	public function run(IOutput $output): void
	{
		if (!$this->backupService->hasDataToBackup()) {
			$output->info('BudgetCheck: no existing tables to back up (fresh install); skipping pre-update snapshot.');
			return;
		}
		...
		try {
			...
			$output->warning('BudgetCheck: pre-update backup failed: ' . $e->getMessage());
			throw $e;
		}

		$tableCount = count($result['manifest']['tables'] ?? []);
		$output->info(sprintf(
			'BudgetCheck: pre-update backup created (%s, %d table(s)). '
			. 'Restore with `occ budgetcheck:upgrade-backup restore --latest --force` if needed.',
			$result['id'],
			$tableCount,
		));
	}
