...
/**
 * Snapshot and restore BudgetCheck data before app updates.
 *
...
use DateTimeZone;
use OCA\BudgetCheck\Exception\UpgradeBackupException;
...
{
	private const LOCK_KEY = 'budgetcheck-upgrade-backup';
	...
	{
		return $this->runExclusive('BudgetCheck upgrade backup', function () use ($reason, $rotate, $rotationExcludeIds): array {
	...
	private function doCreateSnapshot(string $reason, bool $rotate, array $rotationExcludeIds): array
	{
		...
		if (!$this->hasDataToBackup()) {
			throw new UpgradeBackupException('No BudgetCheck tables exist yet; nothing to back up.');
		}
		...
		try {
			...
			if ($backedUpTables === []) {
				throw new UpgradeBackupException('No BudgetCheck tables exist on this instance; nothing to back up.');
			}
		...
		$this->logger->info('BudgetCheck: created pre-update backup snapshot', [
	...
	public function restoreSnapshot(string $snapshotId, bool $createSafetySnapshot = true): void
	{
		...
		$this->runExclusive('BudgetCheck upgrade restore', function () use ($snapshotId, $folder, $tableManifest, $createSafetySnapshot): void {
	...
	private function doRestoreSnapshot(
		...
		try {
			...
			$this->db->rollBack();
			throw new UpgradeBackupException('BudgetCheck restore failed; database changes were rolled back.', 0, $e);
		} finally {
			if ($fkChecksDisabled) {
		...
		try {
			...
			throw new UpgradeBackupException(
				'BudgetCheck database was restored but app config or app data folders failed. '
		...
		$this->logger->warning('BudgetCheck: restored data from upgrade backup snapshot', [
			'app' => UpgradeBackupCatalog::APP_ID,
	...
	private function getAppDataRootFolder(): Folder
	{
		...
		if (!$node instanceof Folder) {
			throw new UpgradeBackupException('BudgetCheck app data root is not a folder.');
		}
	...
	private function getBackupRootFolder(): Folder
	{
		...
		if (!$node instanceof Folder) {
			throw new UpgradeBackupException('BudgetCheck upgrade backup root is not a folder.');
		}
	...
	private function runExclusive(string $label, callable $callback)
	{
		try {
			...
			throw new UpgradeBackupException(
				'Another BudgetCheck backup or restore is already in progress.',
				0,
				$e,
			);
		}
