...
/**
 * List, create, or restore BudgetCheck upgrade backups (CLI operator interface).
 *
 * @copyright Copyright (c) 2026, Nextcloud GmbH
 * @license AGPL-3.0-or-later
 */

namespace OCA\BudgetCheck\Command;

use OCA\BudgetCheck\Exception\UpgradeBackupException;
use OCA\BudgetCheck\Service\UpgradeBackupCatalog;
use OCA\BudgetCheck\Service\UpgradeBackupIntegrity;
...
{
	private const APP_LABEL = 'BudgetCheck';
	...
	{
		$this
			->setName('budgetcheck:upgrade-backup')
			->setDescription('Manage BudgetCheck snapshots taken before app updates.')
			->setHelp(
				<<<'HELP'
...
<info>create</info> — manual snapshot before risky changes:
  <comment>occ budgetcheck:upgrade-backup create --reason="before manual fix"</comment>

<info>restore</info> — destructive; requires <comment>--force</comment> and snapshot id:
  <comment>occ budgetcheck:upgrade-backup restore --latest --force</comment>
  <comment>occ budgetcheck:upgrade-backup restore --id=YYYYMMDDTHHMMSSZ-hex --force</comment>
...
HELP
			...
			->addOption('force', null, InputOption::VALUE_NONE, 'Required for restore — overwrites current BudgetCheck data')
	...
	private function runList(SymfonyStyle $io): int
	{
		...
		if ($snapshots === []) {
			...
			$io->note('Create one manually: occ budgetcheck:upgrade-backup create --reason="before change"');
	...
	private function runRestore(InputInterface $input, SymfonyStyle $io): int
	{
		...
		if (!$input->getOption('force')) {
			$io->error([
				'Restore overwrites all current BudgetCheck database rows, app config, and configured app-data folders.',
		...
		if ($snapshotId === '') {
			$io->error([
				...
				'  occ budgetcheck:upgrade-backup restore --latest --force',
				'  occ budgetcheck:upgrade-backup restore --id=<snapshot-id> --force',
				'Run `occ budgetcheck:upgrade-backup list` to see snapshot ids.',
			]);
			return Command::FAILURE;
		}
