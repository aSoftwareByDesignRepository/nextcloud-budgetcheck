...
final class UpgradeBackupIntegrationTest extends TestCase
{
	...
	{
		if (!$this->db->tableExists('bc_workspaces')) {
			self::markTestSkipped('BudgetCheck tables not present in this instance.');
		}

		$before = $this->countRows('bc_workspaces');
		...
		$this->db->getQueryBuilder()
			->delete('bc_workspaces')
			->executeStatement();
		self::assertSame(0, $this->countRows('bc_workspaces'));

		$this->backupService->restoreSnapshot($snapshotId, false);
		self::assertSame($before, $this->countRows('bc_workspaces'));
	}
