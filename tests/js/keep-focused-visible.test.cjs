...
describe('budgetcheck keep-focused-visible', () => {
	beforeEach(() => {
		_resetPadHostForTests();
	...
	it('desktop focus without soft keyboard never scrolls or pads', () => {
		...
		text.matches = () => true;
		text.closest = (sel) =>
			String(sel).includes('.bc-dialog') || String(sel).includes('[role="dialog"]') || String(sel).includes('.modal')
				? host
				: null;
	...
	it('resolvePadHost prefers dialog body when present', () => {
		...
		const field = {
			closest(sel) {
				if (String(sel).includes('.bc-dialog') || String(sel).includes('[role="dialog"]')) {
					return dialog;
				}
				return null;
			},
