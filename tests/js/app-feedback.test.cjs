#!/usr/bin/env node
/**
 * Behavioural tests for BudgetCheck app-feedback.js (mailto + toast hook).
 *
...
function boot(extra) {
	...
	const config = {
		appId: 'budgetcheck',
		appDisplayName: 'BudgetCheck',
		appVersion: '1.0.0',
		feedbackEmail: 'dev@software-by-design.de',
		githubIssuesUrl: 'https://github.com/aSoftwareByDesignRepository/nextcloud-budgetcheck/issues',
		cssPrefix: 'bc',
		...(extra || {}),
	};
	const cfgEl = {
	...
	nodes.set('bc-app-feedback-config', cfgEl);
	...
	nodes.set('bc-feedback-problem', problem);
	nodes.set('bc-feedback-idea', idea);

	const trigger = {
		className: 'bc-nav-footer__trigger',
		getAttribute(k) {
			return this[k] || null;
		},
		setAttribute(k, v) {
	...
	trigger['aria-controls'] = 'bc-feedback-menu';
	const menu = {
		id: 'bc-feedback-menu',
		hidden: true,
		contains() {
	...
	nodes.set('bc-feedback-menu', menu);
	doc.querySelector = (sel) => {
		if (sel === '.bc-nav-footer__trigger') {
			return trigger;
		}
		return null;
	};
	doc.getElementById = (id) => {
		if (id === 'bc-feedback-menu') {
			return menu;
		}
		return nodes.get(id) || null;
	};
	...
	const sandbox = {
		...
		location: {
			...
			pathname: '/apps/budgetcheck/today',
			search: '?token=secret&keep=1',
			href: 'https://cloud.example/apps/budgetcheck/today?token=secret&keep=1',
		},
...
function main() {
	...
	const clean = sandbox.SbdAppFeedback.sanitizePageUrl(
		'https://cloud.example/apps/budgetcheck/x?token=abc&keep=1&password=no'
	);
	...
	const upgradeAck = sandbox.SbdAppFeedback.sanitizePageUrl(
		'/apps/budgetcheck/?IKnowThatThisIsABigInstanceAndTheUpdateRequestCouldRunIntoATimeoutAndHowToRestoreABackup=IAmSuperSureToDoThis'
	);
	assert(upgradeAck === '/apps/budgetcheck/', 'strips Nextcloud web-upgrade ack query key');
	...
	process.stdout.write('app-feedback.test.js OK (budgetcheck)\n');
}
