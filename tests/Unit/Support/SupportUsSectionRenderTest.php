...
final class SupportUsSectionRenderTest extends TestCase {
	...
	public function testRenderEscapesDisplayNameAndOmitsMobileWithoutFlag(): void {
		$html = $this->renderSection(
			new SupportUsLinks('BudgetCheck', false, null),
			'en'
		);
		...
		self::assertStringContainsString(rawurlencode('BudgetCheck: partner / care retainer'), $html);
	...
	public function testRenderIncludesMobileLicenseWhenConfigured(): void {
		$html = $this->renderSection(
			new SupportUsLinks(
				'BudgetCheck',
				true,
				'/apps/budgetcheck/admin/license'
			),
			'de'
		);
		...
		self::assertStringContainsString('href="/apps/budgetcheck/admin/license"', $html);
		self::assertStringContainsString(rawurlencode('BudgetCheck: Partner / Care Retainer'), $html);
