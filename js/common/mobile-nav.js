/**
 * BudgetCheck mobile navigation: in-page Menu button + slide-in drawer.
 * On mobile open, nav + backdrop portal to document.body so #content
 * overflow cannot clip the drawer (arbeitszeitcheck#33 / Atlas
 * ATLAS_MOBILE_NAV_CONTRACT). Core #app-navigation-toggle alone is not enough.
 *
...
(function () {
	...
	const BACKDROP_ID = 'bc-nav-backdrop';
	...
	function init() {
		...
		const toggle = document.getElementById('bc-nav-toggle');
		...
		const shell = document.querySelector('#app-content.bc-app > #app-content-wrapper.bc-shell')
			|| document.querySelector('#app-content.bc-app > .bc-shell');
		...
		function setOpen(open) {
			...
			nav.classList.toggle('bc-nav--open', open);
			toggle.setAttribute('aria-expanded', open ? 'true' : 'false');
			toggle.setAttribute('aria-label', open ? closeLabel : openLabel);
			document.body.classList.toggle('bc-nav-open', open);
			document.body.classList.remove('snapjs-left');
			...
			if (open) {
				...
				trapHandler = function (event) {
					if (!nav.classList.contains('bc-nav--open') || event.key !== 'Tab') {
						return;
					}
		...
		toggle.addEventListener('click', (event) => {
			...
			setOpen(!nav.classList.contains('bc-nav--open'));
		});
		...
		document.addEventListener('keydown', (event) => {
			if (event.key === 'Escape' && nav.classList.contains('bc-nav--open')) {
				setOpen(false);
				toggle.focus();
			}
		});
		...
		function onViewportChange() {
			...
			if (desktopMq.matches) {
				...
				restoreDomOrder();
				nav.removeAttribute('aria-hidden');
			} else if (!nav.classList.contains('bc-nav--open')) {
				restoreDomOrder();
