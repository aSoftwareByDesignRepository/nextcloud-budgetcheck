 ...
 * @var \OCP\IL10N $l
 * @var \OCA\BudgetCheck\Support\AppFeedbackLinks $appFeedbackLinks optional; constructed when omitted
...
$l = $l ?? (\OCP\Util::getL10N('budgetcheck'));
...
if (!isset($appFeedbackLinks) || !$appFeedbackLinks instanceof AppFeedbackLinks) {
	$appFeedbackLinks = new AppFeedbackLinks('budgetcheck', 'BudgetCheck', $version);
}
$pageUrl = '';
...
>
	<div class="<?php p($prefix); ?>-nav-footer__popover">
		<button
			type="button"
			class="<?php p($prefix); ?>-nav-footer__trigger bc-nav__link"
			aria-expanded="false"
			aria-controls="<?php p($menuId); ?>"
			aria-haspopup="true"
		>
			<span class="bc-icon" aria-hidden="true"><?php
				print_unescaped(IconCatalog::render('info'));
			?></span>
			<span class="bc-nav__name"><?php p($l->t('Help & Feedback')); ?></span>
		</button>
		<ul
